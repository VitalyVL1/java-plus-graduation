...
public abstract class BaseExceptionHandler {
    ...
    public ApiError handleBadRequestException(
        ...
        return new ApiError("BAD_REQUEST", "Переданные не верные параметры", e.getMessage());
    }
    ...
    public ApiError handleServiceTemporaryUnavailable(
            ServiceTemporaryUnavailableException e) {
        log.warn("503 {}", e.getMessage(), e);
        return new ApiError("SERVICE_UNAVAILABLE", "Сервис временно недоступен", e.getMessage());
    }
